...
'''Si lo quisieramos hacer con i seria algo asi
for i in range(1,n)
     
    [ax, ay, az] = a(x[i-1], y[i-1], z[i-1])

    x[i]=x[i-1]+vx[i-1]*dt-0.5*ax*dt**2
    y[i]=y[i-1]+vy[i-1]*dt-0.5*ay*dt**2
    z[i]=z[i-1]+vz[i-1]*dt-0.5*az*dt**2
    
    [ax1, ay1, az1] = a(x[i], y[i], z[i])

    vx[i] = vx[i-1] + 0.5 * (ax + ax1) * dt
    vy[i] = vy[i-1] + 0.5 * (ay + ay1) * dt
    vz[i] = vz[i-1] + 0.5 * (az + az1) * dt
